...
int get_http_sts(char* bufhttp, int buflen)
{
	...
	for (ii = 0; ii < RD_LAST; ii++)
	{
		sprintf(buf, "<tr><td>%s</td> <td>%i</td><td>%i</td> <td>%i</td> <td>%s</td></tr>\n", radiateur[ii].name, radiateur[ii].calculated_target_temp, radiateur[ii].expected_state,
				radiateur[ii].http_req_temp, ctime(&radiateur[ii].http_req_time));
		strcpy(&bufhttp[len], buf);
		len += strlen(buf);
		if (len > buflen - 512)
		{
...
int parse_http_mqtt_temp(char* cmd)
{
	...
	for (ii = 0; ii < TH_LAST; ii++)
	{
		...
		if (thermometer[ii].type == 'S')
		{
			if (strncmp(szID, thermometer[ii].id, 12) == 0)
			{
				char str_value[128];
				char str_topic[256];

				thermometer[ii].temperature = strtof(szTemp, NULL) ;
				thermometer[ii].mesure_date = time(NULL);
				identified++;
				sem_post(&sem_capteur_data_available);
				info("MQTT", "Received thermometer %s: %f", thermometer[ii].name, thermometer[ii].temperature);
