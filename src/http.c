...
void envoie_live_data(FILE* stream, char* chemin, int keepalive)
{
	...
	else if (strcmp(short_name, HTTP_THERMOSTAT) == 0)
	{
		...
		if (status)
		{
			sprintf(bufhttp, "{\"targetHeatingCoolingState\": %i,\"targetTemperature\": %i,\"currentHeatingCoolingState\":%i,\"currentTemperature\": %2.2f,\"currentRelativeHumidity\": %2.2f}", 3,
					radiateur[rad].calculated_target_temp, radiateur[rad].expected_state, thermometer[radiateur[rad].thermometer].temperature, thermometer[radiateur[rad].thermometer].hygrometrie);
		}
		else
		{
