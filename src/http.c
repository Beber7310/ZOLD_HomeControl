...
int cree_socket_ecoute(int port)
{
	...
	if (listen_fd==-1) fatal_error("echec de socket");
...
{
	if (!fgets(buf, size-1, stream)) fin_connection(stream, "echec de fgets");
}
...
void lit_get(FILE* stream, char* url, int size)
{
	...
	if (strncmp(buf, "GET", 3)) fin_connection(stream, "la requete n'est pas GET");
	for (i=3; buf[i]==' '; i++);
...
void * http_loop(void * arg)
{
	...
	while (1) {
		...
		if (errno) fatal_error("Echec de pthread_create");
